...
def search_orders(customer_name: str = Query(..., description='Customer name to search')):
    ...
    if len(results) == 0:
        return {'message': 'No orders found for: ' + customer_name}
...
def browse_products(
    ...
    limit:   int = Query(4,       description='Items per page'),
):
    ...
    if sort_by == 'price' or sort_by == 'name':
        if order == 'desc':
            result = sorted(result, key=lambda p: p[sort_by], reverse=True)
        else:
            result = sorted(result, key=lambda p: p[sort_by], reverse=False)
