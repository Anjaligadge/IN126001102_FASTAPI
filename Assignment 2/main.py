...
orders = []
order_counter = 1
feedback = []   # Q3: stores all feedback
...
# NOTE: /products/filter and /products/summary must come BEFORE /products/{product_id}
...
def filter_products(
    ...
    min_price: int  = Query(None)   # ← Q1: added min_price here
):
    result = products
    ...
    if min_price:            result = [p for p in result if p['price'] >= min_price]  # ← Q1
...
# ── BONUS: Modified place_order — status starts as "pending" ─────────────────
@app.post('/orders')
...
# ── BONUS: Confirm an order (pending → confirmed) ─────────────────────────────
